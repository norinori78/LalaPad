...
- USB-Cケーブル（マグネット式がオススメ
- プラスドライバー
- はんだごて一式
- ケースの滑り止め（グリップラスが評判良いです　https://www.amazon.co.jp/dp/B08C9Z2K4C/

### あると捗るもの
- テンティング用、マグネット式スタンドor三脚orクランプアーム（スマホ用カメラ用などが適合
- テスター：配線のショートをチェック
...
**Q: バッテリーの持ちはどれくらいですか？**  
A: セントラル側が1000mahバッテリー搭載で、毎日デスクワークに使用して2週間前後のバッテリー持ちを確認しています。周辺側はこれの更に数倍持つと思われます。
...
A: 安〇先生、ゲームがしたい・・・です・・・（長押しで出さない生の物理キーとして修飾キーがゲームで必須であることと、WASDを1列右にスライドして使うのもアリだけど、そうするとTGBが消えるので某FPSゲーで困るのです・・・。
...
**Q: トラックパッドの初動がもたつきます**  
A1:  40秒間トラックパッドが操作されないとスリープ状態に入り、再アクティブ化に0.3秒前後かかる仕様になっています。
A2: 　ファームウェアの構成を変更することで、トラックパッドの自動スリープ機能をオフにすることができます。代わりにバッテリー持ちが悪化します。

**Q: キーボードの初動がもたつきます**  
A1:  約１時間キーボードが操作されないとスリープ状態に入り、BLE接続が解除されます。再アクティブ化に５秒前後かかる仕様になっています。
