...
**Q: デバイスとのペアリングができません**  
A0:　PC等デバイス側で周囲の無線機器を検索し、"Lalapad"という機器が表示されるのを確認したらそれを選択し、接続してください。
...
A5:　電源投入時にLEDインジケーターが点灯しているかどうかを確認してください。点灯しない場合、はんだづけの不良をチェックしてください。

**Q: 左手側のキーボードが動作しません**  
A1:　右手用キーボードの電源を投入してから左手キーボードを使用してください。左手側は右手側（セントラル）を経由してPC等デバイスと接続している為です。

A2:　左右キーボードのペアリングに失敗している可能性があります。一度双方の電源を落とし、右手側⇒左手側の順番で電源を投入してみてください。
